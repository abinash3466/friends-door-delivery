...
const FoodPage = () => {
  ...
  useEffect(() => {
    const fetchRestaurants = async () => {
      try {
        const res = await axios.get('/api/admin/stores'); // Admin/owner reads all, client reads active
        if (res.data.success) {
          // Filter to Restaurants and active stores
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        alert('Food order placed successfully! Check your dashboard timeline.');
        setCart({});
        setSelectedStore(null);
        navigate('/customer');
      }
    } catch (error) {
      alert(error.response?.data?.message || 'Order failed. Please check geofencing parameters.');
    } finally {
    ...
    <div className="flex-1 flex flex-col bg-[#FAFAFB] dark:bg-[#121214]">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h2 className="text-xl sm:text-2xl font-black text-slate-800 dark:text-white">Food Delivery</h2>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Fresh meals from Ambasamudram's best kitchens.</p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            ...
            <div className="relative max-w-md">
              ...
              <input
                type="text"
                placeholder="Search food joints or cuisines..."
                value={searchQuery}
            ...
            {stores.length === 0 ? (
              <div className="glass-panel p-8 text-center rounded-2xl text-slate-400">No active restaurants found in the service zone.</div>
            ) : (
                ...
                {stores.map((restaurant) => (
                  ...
                  >
                    ...
                    <div className="p-5">
                      ...
                      <h4 className="font-extrabold text-slate-850 dark:text-white mt-2">{restaurant.name}</h4>
                      <p className="text-xs text-slate-400 mt-1 truncate">Serving best dinners, biryanis and local parottas.</p>
                      <div className="text-xs text-brand-amber font-bold mt-3.5 flex items-center">
                        Open Catalog →
                      </div>
                    </div>
                  </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-6">
              <div className="flex justify-between items-center pb-4 border-b border-slate-100 dark:border-slate-800">
                <div>
                  <h3 className="text-lg font-bold text-slate-850 dark:text-white">{selectedStore.name} Menu</h3>
                  <p className="text-xs text-slate-450 dark:text-slate-400">Location: {selectedStore.locationName}</p>
                </div>
                <button
              ...
              {selectedStore.items?.length === 0 ? (
                <div className="text-slate-400 text-xs py-6">No menu items published yet.</div>
              ) : (
                  ...
                  {selectedStore.items?.map((item) => (
                    <div key={item._id} className="pt-4 flex justify-between items-center">
                      <div className="space-y-1 max-w-[70%]">
                        ...
                        <p className="text-xs text-slate-450 dark:text-slate-400 leading-normal">{item.description}</p>
                      ...
                      <div>
                        {cart[item._id] ? (
                          <div className="flex items-center space-x-3 bg-brand-teal text-white rounded-xl px-3 py-1.5 shadow-md">
                            <button onClick={() => handleRemoveFromCart(item)} className="font-bold hover:scale-110"><Minus className="w-3.5 h-3.5" /></button>
                            <span className="text-xs font-bold">{cart[item._id].quantity}</span>
                            <button onClick={() => handleAddToCart(item)} className="font-bold hover:scale-110"><Plus className="w-3.5 h-3.5" /></button>
                          </div>
                        ) : (
                          <button
            ...
            {/* Shopping Cart details / Checkout Panel */}
            <div className="space-y-6">
              <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider">Checkout Cart</h3>
              ...
              <div className="glass-panel rounded-2xl p-5 border border-slate-100 dark:border-slate-850 space-y-4">
                ...
                {cartItemsArray.length === 0 ? (
                  <div className="text-xs text-slate-450 dark:text-slate-400 text-center py-6">Your cart is empty. Add delicious foods from menu.</div>
                ) : (
                  <>
                    {/* Cart Items list */}
                    ...
                    {cartTotalValue < 200 ? (
                      <div className="p-3 bg-brand-amber/10 border border-brand-amber/25 text-brand-amber-dark rounded-xl flex items-start space-x-2 text-[10px]">
                        ...
                        <span>Minimum order total must be ₹200. Add ₹{200 - cartTotalValue} more items to place order.</span>
                      </div>
                    ) : (
